...
-- This query finds the top 3 most bought products for each product type based on total purchases.
...
ranked_products AS (
    SELECT *,
        RANK() OVER (PARTITION BY product_type ORDER BY total_amount DESC) AS rank
...
-- This query selects the top 3 highest-rated products within each product type.
...
-- This query lists the top 5 most frequently bought products for each generation.
...
-- This query shows the top 5 favorite products by each income group based on customer count.
...
ranked_income_favorites AS (
    SELECT *,
        DENSE_RANK() OVER (PARTITION BY income ORDER BY total_purchase DESC) AS rank
...
-- This query finds the top 5 favorite products for each gender based on customer count.
...
ranked_by_gender AS (
    SELECT *,
        DENSE_RANK() OVER (PARTITION BY gender ORDER BY purchases DESC) AS rank
    FROM ranked_gender_favorites
)
SELECT *
...
-- This query shows the top 10 products by sales among female customers.
...
ranked_spending_female AS (
    SELECT *,
        DENSE_RANK() OVER (PARTITION BY gender ORDER BY total_sales DESC) AS rank
...
WITH seasonal_purchases AS (
    SELECT 
        ...
        SUM(Amount) AS total_amount
    FROM customer_transactions
    GROUP BY product_type, products, EXTRACT(MONTH FROM date)
),
ranked_seasonal_products AS (
    SELECT 
       *,
        RANK() OVER (PARTITION BY season ORDER BY total_amount DESC) AS rank
